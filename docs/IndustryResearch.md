...
业务方通过一个 Agent 目录配置 prompt、模型、Tool、渠道和定时规则，OryxOS 从 `AGENT.md` frontmatter 派生运行配置；业务方写 Tool（用 Java、Python、Shell 或任何语言，通过 **MCP** 协议暴露）接入 OryxOS，Agent 就能使用。业务方不需要为每个 Agent 单独编写后端服务。
...
OryxOS 借鉴了开源 Agent OS 领域已经验证的设计哲学，包括 Agent 配置和生命周期、Channel 抽象、三层记忆、渐进式上下文加载、MCP Tool 和单二进制部署。OryxOS 采用“一目录一 Agent”：`AGENT.md` 定义 Agent，`skills/*.md` 是其可选子指令；目录形态借鉴 Agent Skills，但不把 Agent 本身等同于一个 Skill。OryxOS 在 Java 生态中实现这套底座，并逐步补齐多租户、SSO、RBAC、审计、合规、Web 管理台和企业深度集成。

- **OryxOS 跟 OpenClaw、Hermes 的关系**是同类不同定位。三者都是 Agent OS，OpenClaw 偏个人、Hermes 偏个人到小团队，OryxOS 直接定位严监管企业场景。生态互通通过 Agent Skills 导入适配实现，第三方内容必须经过企业审查后才能进入 OryxOS 工作区。
...
3. **Skill**：可复用的 Agent 能力或子指令。业界常用 `SKILL.md` 描述；OryxOS 核心阶段将 `skills/*.md` 作为所属 Agent 目录内的子指令，并在扩展阶段提供外部 Agent Skills 的导入适配。
