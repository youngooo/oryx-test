...
- 业务方零代码扩展，写 `AGENT.md` + 复用 MCP，用 Markdown 就能定义新场景
...
| 术语 | 定义 |
|------|------|
| **Agent（智能体）** | 一个具象的业务智能体，由 `.oryxos/agents/<name>/` 下的一个自足目录定义。`AGENT.md` 正文描述“做什么”，frontmatter 描述“怎么跑”，可选的 `skills/`、`scripts/` 和参考文件提供按需加载的子指令与资源 |
| **Profile（运行配置）** | OryxOS 内核使用的运行时对象，由 `AgentLoader` 从 `AGENT.md` frontmatter 派生，决定 Provider、模型、可用 Tool、Channel、通知目标、定时规则和循环参数；用户不再单独维护 Profile YAML |
...
| **Tool（工具）** | Agent 可以调用的外部能力。内置 Tool 包括文件、Shell、HTTP、Memory 和通知推送；Plugin Tool 通过 MCP server 或 Java `@Tool` Bean 扩展 |
...
| **Tool Policy（工具策略）** | 控制 Agent 可用工具的允许或拒绝规则；核心阶段用 `AGENT.md` frontmatter 的 `tools` 字段限定工具子集，完整 allow/deny 策略放在扩展阶段 |
| **Skill（子指令）** | Agent 目录中可选的 `skills/*.md` 文件，只服务于所属 Agent，由 `AGENT.md` 正文引导模型通过 `read_file` 按需读取；核心阶段没有全局 Skill 索引或跨 Agent 共享能力库 |
| **Bootstrap（引导文件）** | 加载到系统提示词中的上下文文件：`AGENTS.md`（项目级 Agent 行为说明）、`SOUL.md`（Agent 人格定义）、`USER.md`（用户偏好） |
| **Workspace（工作区）** | OryxOS 实例的工作目录，默认是 `.oryxos/`，包含 Agent 目录、Bootstrap、Memory、MCP 配置、日志和 SQLite 数据库 |
...
- 三个 Bootstrap 文件在 Agent 运行时被加载到系统提示词，让 Agent 知道项目背景、默认人格和用户偏好。
- `oryxos init` 同时生成一个最小可用的 `default` Agent 目录。

---

### 5.2 Agent 目录与派生 Profile

一个业务 Agent 由 `.oryxos/agents/<name>/` 目录定义。目录中至少包含 `AGENT.md`；其 frontmatter 描述运行配置，正文描述任务指令。可选的 `skills/`、`scripts/` 和 `REFERENCE.md` 按需提供子指令、确定性脚本和参考材料。

底座仍使用 `Profile` 运行时对象，但它由 `AgentLoader.deriveProfile(agentDir)` 从 frontmatter 派生，用户不再单独维护 `.oryxos/profiles/*.yaml`。

**`AGENT.md` frontmatter 示例：**

```yaml
name: string                    # Agent/派生 Profile 名称
...
实际文件用 `---` 包围上述 YAML；frontmatter 之后的 Markdown 正文描述 Agent 的任务、边界，以及对子指令或脚本的使用说明。

**Agent 管理命令：**

```bash
oryxos profile create <name>    # 创建 Agent 目录和 AGENT.md
oryxos profile list             # 查看所有 Agent/派生 Profile
oryxos profile show <name>      # 查看 Agent 定义和派生配置
oryxos profile delete <name>    # 删除或归档 Agent 目录
```

`profile` 是为保持 CLI 稳定而保留的命令组名称。核心阶段支持多个 Agent 在同一个 OryxOS 实例中并存，这是“OS”在核心阶段的最小体现。
...
| `notify` | 通知 | 向 Agent 配置的 webhook 通知目标推送内容 |

#### Plugin Tool（业务方扩展）

| 方式 | 门槛 | 推荐度 | 场景 |
|------|------|--------|------|
| **方式一**：写 Agent 目录 + 复用 MCP server | 零代码 | ⭐⭐⭐ 主推 | 用 `AGENT.md` 描述意图，LLM 自己组合现成能力 |
...
1. 创建 `daily-pr-digest/AGENT.md`，在 frontmatter 声明运行配置，在正文描述任务
...
核心阶段 Session 数据持久化到 `.oryxos/oryxos.db`。重启 OryxOS 后，正在进行的 Session 可以恢复。Session 上下文超过 LLM 的 context window 时，简单截断早期对话并保留近期对话。
...
三种模式共享同一套 Agent 定义、派生 Profile 和 Session 存储。
...
- 敏感配置通过**环境变量**注入或独立的本地配置文件加载，不明文写进 `AGENT.md`
- `AGENT.md` frontmatter 和 MCP 配置中可用 `${ENV_VAR}` 占位，加载时从环境变量解析
...
- **Skill 生态兼容**：完善 Agent 目录与外部 Agent Skills 规范之间的导入、校验和复用机制
...
- `AGENT.md` 正文和 Bootstrap 修改在下一轮 Prompt 组装时生效；新增 Agent 或修改定时配置在核心阶段通过重载工作区或重启服务生效
...
```text
用户执行 oryxos init
  → OryxOS 创建 .oryxos/ 工作区
  → 创建三个 Bootstrap 文件（AGENTS.md、SOUL.md、USER.md）
  → 创建 memory/MEMORY.md、mcp_servers.yaml 和默认 Agent（agents/default/AGENT.md）
用户编辑 Bootstrap 文件填入项目背景、Agent 人格、用户偏好
用户编辑 default/AGENT.md 选择 Provider 和模型
用户通过环境变量配置 API key
```
...
用户执行 oryxos profile create <name>
  → OryxOS 创建 .oryxos/agents/<name>/AGENT.md
用户编辑 frontmatter 配置 Provider、Tool、Channel、通知目标和定时规则
用户编辑正文描述 Agent 的任务和行为边界
用户执行 oryxos chat --profile <name>
  → AgentLoader 读取 Agent 目录并派生 Profile
  → 初始化 Provider 连接
  → 注册 Tool 到 Agent 工具池
  → 把 AGENT.md 正文和 Bootstrap 文件加载到系统提示词
  → Agent 进入待对话状态
```
...
```text
消息从 CLI、HTTP API 或 AgentScheduler 进入
  → 统一调用 AgentService
  → Agent 查询 Session 上下文
  → 组装 LLM Prompt（AGENT.md 正文 + Bootstrap + Memory + 对话历史 + 可用 Tool 列表）
  → 调用 LLM Provider 获取响应
  → [有 Tool 调用] → OryxOS 执行 Tool → 结果回传给 LLM 继续生成
  → 最终响应返回调用方或由 notify Tool 主动推送
  → LLM 和 Tool 调用写入审计表并输出结构化日志
```
...
Agent 的持久化定义是 `.oryxos/agents/<name>/` 目录；运行时 Profile 由 `AgentLoader` 从 `AGENT.md` frontmatter 派生。见 [5.2 Agent 目录与派生 Profile](#52-agent-目录与派生-profile)。
...
- `oryxos init` 工作区初始化、`AGENT.md` frontmatter 解析与 Profile 派生
...
- 多 Agent 演示（两个不同 Agent 目录在同一实例并存）
...
| **生态关系风险** | OryxOS 和 OpenClaw、Hermes 的关系 | Agent 目录借鉴开放的 Agent Skills 形态，并通过导入适配实现生态互通；OpenClaw 偏个人、Hermes 偏小团队、OryxOS 定位企业 |
...
- [ ] `oryxos init` 工作区初始化
- [ ] Agent 目录加载与 Profile 派生（支持多 Agent 并存）
...
- [ ] Plugin Tool 接入（方式一零代码 `AGENT.md` + 现成 MCP 跑通；方式二自写 MCP server 跑通；方式三 Java `@Tool` Bean 示例跑通）
...
三个**每日自动运行**的端到端 Demo 分别验证 Agent 目录的三种丰富度，并共同覆盖五大核心能力和定时任务。三个 Demo 跑通是核心功能发布的硬条件：
...
| **Demo 二：每日科技日报** | `AGENT.md` + `skills/`、MCP、Memory、通知、定时任务 | 每天到点自动汇总科技新闻并推送，内容体现用户之前说过的关注方向 | 业务方只写 Agent 目录和 MCP 配置；LLM 通过 `read_file` 按需读取组稿子指令，自主调用新闻工具并组织日报；日报体现 `MEMORY.md` 中的偏好 |
| **Demo 三：每日 GitHub 日报** | `AGENT.md` + `scripts/`、Shell Tool、Memory、通知、定时任务 | 每天自动抓取 GitHub 热门项目，突出 AI 相关内容并推送 | Agent 通过 `shell` 执行所属目录中的脚本；解释器和脚本路径通过白名单；脚本输出进入上下文、脚本代码不预载；调用过程写入 `tool_invocations` |

三个 Demo 都是“钟推”（`AgentScheduler` 到点自动触发），同时必须支持通过 `oryxos chat` 或 `POST /api/v1/agents/{name}/invoke` 人工补跑，验证所有入口复用同一条 `AgentService` 链路。
...
- **Plugin 自定义工具 + 内置工具集**：内置文件、Shell、HTTP、Memory 和通知工具；业务方通过 `AGENT.md` + 现成 MCP 零代码扩展、自写 MCP server 轻代码扩展、Java `@Tool` Bean 重代码扩展
- **Web Service**：REST API 覆盖会话管理、Agent 调用、Profile/Memory/Tool 信息查询、系统状态

**核心理念**：OryxOS 五大能力扎实落地，业务方组合 Agent 目录与 MCP server 就能解决业务问题，通过 Web Service 接入已有系统，不需要为每个 Agent 单独编写后端服务。OryxOS 不绑定具体业务，业务方按自己的需求组合。
