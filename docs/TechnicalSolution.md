...
> **文档结构提示：** 全文分三部分。第一部分（第 1-10 章）是**底座**——让任意 Agent 都能可靠运行的引擎、能力和支撑设施，本身不是某个具体的业务 Agent。第二部分（第 11 章）讲底座之上怎么通过一个 Agent 目录定义业务 Agent：`AGENT.md` 正文定义做什么，frontmatter 派生运行 Profile，Web Service 是扩展阶段的管理入口。第三部分（第 12-15 章）是整合验证、实施节奏和收尾。
...
5. **Picocli**（命令行工具）
6. **SnakeYAML**（`AGENT.md` frontmatter YAML 解析）
...
**三档后端实现（核心阶段一次交付，靠配置 `memory.backend` 选一个）：**
...
- **`Mem0MemoryStore`。** 接一个**自托管** Mem0 记忆层（数据不出域），Java 侧走 REST 集成，`append/load/recall` 翻译成 Mem0 的 add/get/search——提炼、冲突消解、语义检索都交给 Mem0。凭证与地址走环境变量占位。这是确实需要智能记忆时的外部集成档。
...
- 知识图谱后端（门槛比向量检索更高，确有需要时集成 Graphiti 一类可自托管方案，不自行实现）
...
**`GlobalExceptionHandler` 模块。** 统一异常处理，把异常转成标准 JSON 响应信封 `ApiResponse`（`code`、`message`、`data`、`timestamp`；成功与错误共用一个信封），所有 Controller 复用，不重复定义错误响应类型。
...
> **核心阶段为什么不做：** 核心阶段通过文件系统定义 Agent：业务方手写 `.oryxos/agents/<name>/` 目录。已加载 Agent 的正文与子资源由 `ContextLoader` 每次现取，内容修改下一轮生效；新增、删除 Agent 或修改 frontmatter 与调度定义，需要重载工作区或重启。通过 API 动态创建 Agent 还需要目录上传、一句话生成、运行时注册和调度增删能力，必须作为完整闭环在扩展阶段一起交付。
...
创建目录、写默认模板，并生成可派生默认 Profile 的 `agents/default/AGENT.md`。
...
**状态持久化与可管理（扩展阶段）。** 光“到点自动跑”还不够——运营方还需要查看任务、执行历史、手动补跑和临时停用。扩展阶段把任务状态和执行历史落 SQLite，并提供管理接口：
...
- **四个管理端点**（`ScheduleApiController`，前缀 `/api/v1/schedules`）：`GET /schedules` 列任务与状态、`GET /schedules/{id}/executions` 查执行历史、`POST /schedules/{id}/run` 立即执行一次、`PUT /schedules/{id}` 启用或停用。

**核心阶段与扩展阶段的边界。** 核心阶段的 `schedules` 定义只能写在 `AGENT.md` frontmatter 中，启动或显式重载工作区时注册；修改 cron 或新增任务后需要重载或重启。核心阶段负责到点触发、进程内防重叠以及调用审计。任务状态持久化、执行历史查询、启停控制、通过 API 增删改调度定义，以及 Agent 目录的上传与实时监听均属于扩展阶段。
...
三种模式共享同一套 Agent 定义、派生 Profile 和 Session 存储，差异只是接入层。
...
**`ConfigLoader` 模块** 负责统一加载 LLM API key、Provider 凭证、MCP server 凭证等敏感配置。核心阶段做基础版：敏感配置通过环境变量注入或独立的本地配置文件加载，不明文写入 `AGENT.md` 或 `mcp_servers.yaml`（配置中用 `${ENV_VAR}` 占位，加载时从环境变量解析）；配置加载时做必填项和格式的基础校验，缺失或非法时给出清晰报错。完整的加密存储、密钥轮转、对接企业 KMS/Vault 放扩展阶段。
...
4. **`scheduled_tasks`**：定时任务登记信息与运行状态
5. **`task_executions`**：定时任务每次执行的历史，成功失败都记

> `tool_invocations` 和 `llm_calls` 在核心阶段就写入，因为可审计的数据地基应从第一天建立。查询接口和审计报表放扩展阶段。`scheduled_tasks` 与 `task_executions` 在扩展阶段把定时任务升级为可查、可管的一等资源。
...
核心阶段启动时扫描全部 Agent 目录；`AGENT.md` 正文和子资源每次现取，因此内容修改下一轮即可生效。新增或删除 Agent 目录、修改 frontmatter 或调度定义后，需要显式重载工作区或重启服务。
...
写完 Agent 目录后走的 `AgentLoader.deriveProfile → ProfileRegistry.register → AgentScheduler.registerProfile`，与启动扫描复用同一段注册逻辑，保证 API 创建与手工放入目录的 Agent 行为一致。扩展阶段补齐 `ProfileRegistry` 和 `AgentScheduler` 的运行时增删能力；`generate` 复用既有 `ProviderService`，并写入 `llm_calls` 审计。
...
- 搭 Maven 多模块骨架（9 个模块）、`oryxos init`、`AGENT.md` frontmatter 解析与 Profile 派生
...
五大能力加支撑模块是**底座**（第一部分），本身不是某个具体的业务 Agent。真正定义业务 Agent 靠一个自足目录：`AGENT.md` 正文说明做什么，frontmatter 派生运行 Profile，可选 `skills/` 与 `scripts/` 提供按需资源。核心阶段通过文件系统维护，扩展阶段统一到 `POST /api/v1/agents` 管理入口（第二部分，第 11 章）。
