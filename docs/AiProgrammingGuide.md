...
| acceptance criteria | 需求文档第 13 章 3 个端到端验收 Demo | 直接复用 |
...
| 5 | Plugin Tool 三档接入 | 主推 `AGENT.md` + 现成 MCP 的零代码方式 |
...
五个 user story 最终通过需求文档第 13 章的三个端到端 Demo 联合验收：

| User Story | 主要对应 Demo | 场景 |
|-----------|--------------|------|
| US-1 + US-2 | Demo 一：每日天气 | 定时触发 ReAct，查询天气并生成穿搭建议 |
| US-3 | Demo 二、三 | 日报体现长期记忆中的用户偏好 |
| US-4 | Demo 一、二、三 | 分别验证内置 Tool、`skills/` + MCP、`scripts/` + Shell |
| US-5 | 三个 Demo 的 API 侧验证 | 查询 Session、Agent、Tool、Memory 与系统状态 |
...
| Agent 定义解析类 | SnakeYAML 解析 `AGENT.md` frontmatter、派生并校验 Profile |
...
**阶段验收（对应最终 Demo 一：每日天气）**

先通过 `oryxos chat` 人工触发天气 Agent，验证 ReAct 循环能够调用 HTTP Tool、累积 Session 并生成穿搭建议；`AgentScheduler` 与通知推送在后续 user story 完成后接入同一个 Agent，形成最终自动运行的 Demo。
...
1. 零代码 Agent 目录（`AGENT.md`）+ 现成 MCP（主推）
2. 轻代码自写 MCP server
3. 重代码 Java `@Tool` 注解
...
- `oryxos-core`（`AGENT.md` 正文和 Agent 目录上下文的加载归 `ContextLoader`，不在 tool 模块）
...
| Agent 目录上下文类 | `ContextLoader` 加载 `.oryxos/agents/` 下每个 Agent 的 `AGENT.md` 正文；`skills/`、`scripts/` 和参考文件由模型按需读取，这部分归 core 不归 tool |
...
- 每日科技日报：Agent 按 `AGENT.md` 指引，通过 `read_file` 按需读取 `skills/digest-format.md`，调用新闻 MCP，并结合 Memory 生成日报。
...
每个 user story 完成后都有阶段性可演示结果，最终汇入需求文档第 13 章的三个端到端验收 Demo。
