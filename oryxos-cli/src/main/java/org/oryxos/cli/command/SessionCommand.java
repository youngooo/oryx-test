...
@Command(name = "session", description = "Inspect persisted Sessions")
public final class SessionCommand implements Runnable {
    @Override public void run() {
    }

    @Command(name = "list", description = "List persisted Sessions")
    public static final class ListCommand implements Callable<Integer> {
        ...
        @Override public Integer call() {
            output.value(launcher.sessions(workspace), json);
            return 0;
        }
    }
}
